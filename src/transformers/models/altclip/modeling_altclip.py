...
from .configuration_altclip import (
    ...
    AltCLIPVisionConfig,
    AltRobertaConfig,
    AltRobertaTextConfig,
    AltRobertaVisionConfig,
)
...
class AltRobertaTextEmbeddings(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings."""
...
class AltRobertaVisionEmbeddings(nn.Module):
    def __init__(self, config: AltRobertaVisionConfig):
        super().__init__()
        self.config = config
        self.embed_dim = config.hidden_size
        self.image_size = config.image_size
...
class AltRobertaTextSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):
            raise ValueError(
...
class AltRobertaTextSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class AltRobertaTextAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = AltRobertaTextSelfAttention(config)
        self.output = AltRobertaTextSelfOutput(config)
...
class AltRobertaVisionAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: AltRobertaVisionConfig | AltRobertaTextConfig):
        super().__init__()
        self.config = config
        self.embed_dim = config.hidden_size
        self.num_heads = config.num_attention_heads
...
class AltRobertaTextIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.intermediate_size)
        if isinstance(config.hidden_act, str):
...
class AltRobertaTextOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.intermediate_size, config.hidden_size)
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class AltRobertaVisionMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.activation_fn = ACT2FN[config.hidden_act]
...
class AltRobertaTextLayer(GradientCheckpointingLayer):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = AltRobertaTextAttention(config)
        self.intermediate = AltRobertaTextIntermediate(config)
        self.output = AltRobertaTextOutput(config)
...
class AltRobertaVisionLayer(GradientCheckpointingLayer):
    def __init__(self, config: AltRobertaConfig):
        super().__init__()
        self.embed_dim = config.hidden_size
        self.self_attn = AltRobertaVisionAttention(config)
        self.layer_norm1 = nn.LayerNorm(self.embed_dim, eps=config.layer_norm_eps)
        self.mlp = AltRobertaVisionMLP(config)
        self.layer_norm2 = nn.LayerNorm(self.embed_dim, eps=config.layer_norm_eps)
...
class AltRobertaTextPooler(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.activation = nn.Tanh()
...
class AltRobertaPreTrainedModel(PreTrainedModel):
    config: AltRobertaConfig
    base_model_prefix = "alt_roberta"
    input_modalities = ("image", "text")
    _no_split_modules = ["AltRobertaVisionEmbeddings", "AltRobertaTextEmbeddings", "AltRobertaVisionAttention"]
    ...
    _supports_attention_backend = True
    _can_record_outputs = {
        "hidden_states": AltRobertaVisionLayer,
        "attentions": AltRobertaVisionAttention,
    }
    ...
    def _init_weights(self, module):
        """Initialize the weights"""
        factor = self.config.initializer_factor
        if isinstance(module, AltRobertaVisionEmbeddings):
            factor = self.config.initializer_factor
            init.normal_(module.class_embedding, mean=0.0, std=module.embed_dim**-0.5 * factor)
            init.normal_(module.patch_embedding.weight, std=module.config.initializer_range * factor)
            init.normal_(module.position_embedding.weight, std=module.config.initializer_range * factor)
            init.copy_(module.position_ids, torch.arange(module.num_positions).expand((1, -1)))
        elif isinstance(module, AltRobertaTextEmbeddings):
        ...
        elif isinstance(module, AltRobertaVisionAttention):
            factor = self.config.initializer_factor
            in_proj_std = (module.embed_dim**-0.5) * ((2 * module.config.num_hidden_layers) ** -0.5) * factor
            out_proj_std = (module.embed_dim**-0.5) * factor
            init.normal_(module.q_proj.weight, std=in_proj_std)
            ...
            init.normal_(module.v_proj.weight, std=in_proj_std)
            init.normal_(module.out_proj.weight, std=out_proj_std)
        elif isinstance(module, AltRobertaVisionMLP):
            factor = self.config.initializer_factor
            in_proj_std = (module.config.hidden_size**-0.5) * ((2 * module.config.num_hidden_layers) ** -0.5) * factor
            fc_std = (2 * module.config.hidden_size) ** -0.5 * factor
            init.normal_(module.fc1.weight, std=fc_std)
            init.normal_(module.fc2.weight, std=in_proj_std)
        elif isinstance(module, AltRobertaModel):
            init.normal_(
                module.text_projection.weight,
                std=module.text_embed_dim**-0.5 * self.config.initializer_factor,
            )
...
class AltRobertaTextEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([AltRobertaTextLayer(config) for i in range(config.num_hidden_layers)])
        self.gradient_checkpointing = False
...
class AltRobertaVisionEncoder(nn.Module):
    """
    Transformer encoder consisting of `config.num_hidden_layers` self attention layers. Each layer is a
    [`AltRobertaVisionEncoderLayer`].
    ...
    def __init__(self, config: AltRobertaConfig):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([AltRobertaVisionLayer(config) for _ in range(config.num_hidden_layers)])
        self.gradient_checkpointing = False
...
@auto_docstring(
    custom_intro="""
    The vision model from ALT_ROBERTA without any head or projection on top.
    """
)
class AltRobertaVisionModel(AltRobertaPreTrainedModel):
    config: AltRobertaVisionConfig
    main_input_name = "pixel_values"
    input_modalities = ("image",)
    _no_split_modules = ["AltRobertaEncoderLayer"]
    _input_embed_layer = "patch_embedding"
    ...
    def __init__(self, config: AltRobertaVisionConfig):
        ...
        self.embeddings = AltRobertaVisionEmbeddings(config)
        self.pre_layrnorm = nn.LayerNorm(embed_dim, eps=config.layer_norm_eps)
        self.encoder = AltRobertaVisionEncoder(config)
        self.post_layernorm = nn.LayerNorm(embed_dim, eps=config.layer_norm_eps)
        self.post_init()
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> from PIL import Image
        >>> import requests
        >>> from transformers import CLIPProcessor, AltRobertaVisionModel

        >>> model = AltRobertaVisionModel.from_pretrained("OFA-Sys/chinese-clip-vit-base-patch16")
...
@auto_docstring(
    custom_intro="""
    The text model from ALT_ROBERTA without any head or projection on top.
    """
)
class AltRobertaTextModel(AltRobertaPreTrainedModel):
    """
    ...
    config: AltRobertaTextConfig
    input_modalities = ("text",)

    _input_embed_layer = "word_embeddings"
    _can_record_outputs = {
        "hidden_states": AltRobertaTextLayer,
        "attentions": AltRobertaTextSelfAttention,
    }
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = AltRobertaTextEmbeddings(config)
        self.encoder = AltRobertaTextEncoder(config)
        self.pooler = AltRobertaTextPooler(config) if add_pooling_layer else None
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> from transformers import AutoTokenizer, AltRobertaTextModel

        >>> model = AltRobertaTextModel.from_pretrained("openai/alt_roberta-vit-base-patch32")
...
def alt_roberta_loss(similarity):
...
@auto_docstring
class AltCLIPModel(AltRobertaPreTrainedModel):
    config: AltRobertaConfig

    def __init__(self, config: AltCLIPConfig):
        super().__init__(config)

        if not isinstance(config.text_config, AltRobertaTextConfig):
            raise TypeError(
                "config.text_config is expected to be of type AltCLIPTextConfig but is of type"
                f" {type(config.text_config)}."
            )

        if not isinstance(config.vision_config, AltRobertaVisionConfig):
            raise TypeError(
        ...
        self.vision_model = AltRobertaVisionModel._from_config(vision_config)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        loss = None
        if return_loss:
            loss = alt_roberta_loss(logits_per_text)

        return AltRobertaOutput(
            loss=loss,
