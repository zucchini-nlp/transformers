...
class WhisperModelIntegrationTests(unittest.TestCase):
    ...
    def test_small_longform_timestamps_generation(self):
        ...
        self.assertEqual(transcript["offsets"], [])
    ...
    def test_small_token_timestamp_generation(self):
        ...
        # fmt: off
        EXPECTED_OUTPUT = torch.tensor([
            [0.0000, 0.7400, 0.8000, 0.9800, 1.0200, 1.1400, 1.4000, 1.5200, 1.9200, 2.2600, 2.3800, 2.5400, 2.8600, 3.2600, 3.3400, 3.4400, 3.6000, 3.6800, 3.9200, 4.2000, 4.4800, 4.7800, 5.2600, 6.7800, 6.7800, 6.7800, 6.7800, 6.7800, 6.7800, 6.7800, 6.7800, 6.7800, 6.7800, 6.7800, 6.7800, 6.7800, 6.7800, 6.7800, 6.7800, 6.7800, 6.7800],
            [0.0000, 0.7600, 0.9800, 1.3000, 1.3800, 1.5200, 1.5800, 1.7000, 1.8400, 2.1000, 2.5000, 3.1400, 3.4400, 3.7400, 4.2000, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400, 5.2400],
            [0.0000, 0.6600, 0.9000, 1.2200, 1.5200, 1.7600, 2.0200, 2.4000, 2.9200, 3.1800, 3.3200, 3.6200, 4.1000, 4.3600, 4.7800, 5.1200, 5.3400, 5.7200, 6.0600, 6.2000, 6.2000, 6.2000, 6.5000, 6.9000, 7.6400, 8.0000, 8.2400, 8.5200, 8.7400, 9.0800, 9.4000, 9.5400, 9.9400, 10.4200, 10.7600, 11.1200, 11.4400, 11.5800, 11.8600, 12.7000, 12.7000],
            [0.0000, 0.6600, 0.8600, 1.1400, 1.5000, 1.9600, 2.3600, 2.6400, 2.9800, 3.1200, 3.2400, 3.4800, 3.7800, 4.1600, 4.6400, 5.0800, 5.4400, 6.2200, 6.2200, 6.2200, 6.4000, 6.8400, 7.1200, 7.2600, 7.4800, 7.8200, 8.1400, 8.7000, 9.0200, 9.0200, 9.2000, 10.1600, 10.1600, 10.1600, 10.1600, 10.1600, 10.1600, 10.1600, 10.1600, 10.1600, 10.1600]
        ])
        # fmt: on
    ...
    def test_tiny_specaugment_librispeech(self):
        ...
        EXPECTED_LOGITS = torch.tensor(
            [
                1.0171, -4.1832, 5.5047, 3.9045, 0.9463, -5.7744, 4.6256, -3.3783,
                -0.6665, 2.7998, 5.6761, 4.0699, 1.0307, -4.7561, 3.0540, 8.1075,
                3.7587, 1.2436, 10.7901, -2.1355, 1.7736, -7.6625, -0.8791, -7.4682,
                1.2088, 4.1190, 4.9652, -4.1093, -1.9641, 9.5019
            ]
        )
        # fmt: on
        torch.testing.assert_close(logits[0][0, 0, :30].cpu(), EXPECTED_LOGITS, rtol=1e-4, atol=1e-4)
    ...
    def test_tiny_static_generation_long_form(self):
        ...
        # See PR #47995 for details. torch.compile (used by static cache) can introduce tiny fp
        # differences vs eager mode that flip argmax at some positions, causing sequences to diverge
        # after ~85-138 generated tokens. The first 40 generated tokens are identical; compare only
        # those to verify basic correctness.
        ...
        self.assertTrue((static_generated_ids[permutation_idx, :] == static_generated_ids_reordered).all())
