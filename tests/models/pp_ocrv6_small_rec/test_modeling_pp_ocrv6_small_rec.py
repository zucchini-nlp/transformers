...
"""Testing suite for the PPOCRV6SmallRec model."""
...
class PPOCRV6SmallRecModelTester:
    def __init__(
        self,
        parent,
        batch_size=3,
    ...
    def get_config(self) -> PPOCRV6SmallRecConfig:
        backbone_config = {
            ...
            "block_configs": [
                [[3, 16, 16, 1, True]],
                [[3, 16, 16, 1, False], [3, 16, 16, 1, False]],
                [
                    [3, 16, 16, [2, 1], False],
                    [3, 16, 16, 1, True],
                    [3, 16, 16, 1, False],
                ...
                [[3, 16, 16, [2, 1], False], [3, 16, 16, 1, True], [3, 16, 16, 1, False]],
            ],
        }
...
@require_torch
class PPOCRV6SmallRecModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (PPOCRV6SmallRecForTextRecognition,) if is_torch_available() else ()
    pipeline_model_mapping = (
        {"image-feature-extraction": PPOCRV6SmallRecForTextRecognition} if is_torch_available() else {}
    )
    ...
    def setUp(self):
        self.model_tester = PPOCRV6SmallRecModelTester(self)
    ...
    @unittest.skip("PPOCRV6SmallRec does not has no attribute `hf_device_map`")
    def test_model_parallelism(self):
        pass

    @unittest.skip("PPOCRV6SmallRec does not has no function `get_input_embeddings`")
    def test_model_get_set_embeddings(self):
        pass

    @unittest.skip(reason="PPOCRV6SmallRec does not support attention")
    ...
    # PPOCRV6SmallRec have no seq_length
    def test_hidden_states_output(self):
        # PP-OCRV6 uses PPLCNetV4 backbone: hidden_states = (embedding, stage1, ..., stageN) = num_stages + 1
        ...
        def check_hidden_states_output(inputs_dict, config, model_class):
            ...
            # First hidden state is 4x downsampled
            self.assertListEqual(
                list(hidden_states[0].shape[-2:]),
                [self.model_tester.image_size[0] // 4, self.model_tester.image_size[1] // 4],
            )
...
@require_vision
@slow
class PPOCRV6SmallRecModelIntegrationTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_inference_text_recognition_head(self):
        ...
        expected_score = 0.9993069767951965
